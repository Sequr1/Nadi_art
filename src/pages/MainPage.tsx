...
export default function MainPage() {
  ...
  return (
    <div className="min-h-screen bg-milk overflow-x-hidden relative">
      ...
      {/* Видео блок */}
      <section className="relative w-full h-[60vh] md:h-screen overflow-hidden bg-milk">
        <video
          autoPlay
          loop
          muted
        ...
        {/* Воздушный туманный переход */}
        <div className="absolute inset-0 pointer-events-none z-10">
          {/* Основной градиентный туман сверху и снизу - делаем более узким */}
          <div className="absolute top-0 left-0 right-0 h-[15%] bg-gradient-to-b from-milk via-milk/80 to-transparent backdrop-blur-[1px]" />
          <div className="absolute bottom-0 left-0 right-0 h-[15%] bg-gradient-to-t from-milk via-milk/80 to-transparent backdrop-blur-[1px]" />
          
          {/* Движущиеся слои тумана - делаем прозрачнее и меньше */}
          <div className="absolute inset-0 overflow-hidden">
            <div className="absolute -inset-[30%] bg-[radial-gradient(circle_at_center,rgba(253,252,250,0.6)_0%,transparent_70%)] blur-[80px] animate-fog-drift opacity-30" />
            <div className="absolute -inset-[30%] bg-[radial-gradient(circle_at_center,rgba(253,252,250,0.4)_0%,transparent_60%)] blur-[100px] animate-fog-drift opacity-25" style={{ animationDelay: '-10s', animationDirection: 'reverse' }} />
          </div>

          {/* Мягкая виньетка для глубины - еще прозрачнее */}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_40%,rgba(253,252,250,0.15)_100%)]" />
        </div>

        {/* Декоративные частицы тумана - более точечно */}
        <div className="absolute inset-0 pointer-events-none z-20">
          <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-white/20 rounded-full blur-[80px] animate-breathe opacity-20" />
          <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-lavender-soft/10 rounded-full blur-[100px] animate-float opacity-15" />
        </div>
      </section>
      {/* Блок "Обо мне" */}
              ...
              <div key={index} className={`rounded-3xl transition-all duration-700 ease-out overflow-hidden ${expandedFact === index ? 'bg-white shadow-xl shadow-lavender/20' : 'bg-white/60 hover:bg-white hover:shadow-lg hover:shadow-lavender/10'}`}>
                <div onClick={() => { setExpandedFact(expandedFact === index ? null : index); setExpandedSubFact(null); }} className="p-6 cursor-pointer">
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className={`font-serif text-xl transition-colors duration-500 flex-1 break-words ${expandedFact === index ? 'text-amethyst' : 'text-text-primary'}`}>{fact.title}</h3>
                    {expandedFact === index && (
