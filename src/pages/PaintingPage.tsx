      ...
      <div className="relative z-10 min-h-screen pt-24 pb-16 px-6 md:px-12">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center max-w-2xl mx-auto space-y-6 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl text-text-primary break-words">{painting.title}</h1>
            {painting.feeling && <p className="text-lavender text-xl md:text-2xl italic font-light break-words">{painting.feeling}</p>}
            {painting.description && <p className="text-text-secondary font-light leading-relaxed text-lg break-words">{painting.description}</p>}
            <div className="flex items-center justify-center gap-4 text-text-muted text-sm flex-wrap">
              {painting.technique && <span className="break-words">{painting.technique}</span>}
              {painting.dimensions && <span className="break-words">· {painting.dimensions}</span>}
              {painting.year && <span className="break-words">· {painting.year}</span>}
            </div>
          </div>
